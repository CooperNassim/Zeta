...
(async () => {
  ...
  try {
    ...
    // 1. 创建新的触发器函数 - 支持中英文
    console.log('1. 创建新的触发器函数...');
    await client.query(`
      CREATE OR REPLACE FUNCTION sync_trade_order_to_records()
      RETURNS TRIGGER AS $$
      BEGIN
          -- 处理买入订单 (支持 '买入' 和 'buy')
          IF (NEW.order_type = '买入' OR NEW.order_type = 'buy') AND NEW.deleted = false THEN
              -- 插入新的交易记录
              ...
              RAISE NOTICE '触发器: 已创建买入交易记录 (order_type=%)', NEW.order_type;
              RETURN NEW;
          END IF;
          
          -- 处理卖出订单 (支持 '卖出' 和 'sell')
          IF (NEW.order_type = '卖出' OR NEW.order_type = 'sell') AND NEW.deleted = false THEN
              DECLARE
              ...
              BEGIN
                  ...
                  IF existing_record IS NOT NULL THEN
                      ...
                      RAISE NOTICE '触发器: 已更新卖出交易记录 (order_type=%)', NEW.order_type;
                  ELSE
                      -- 创建新记录（只有卖出信息）
                      INSERT INTO trade_records (
                          trade_number,
    ...
    console.log('\n✅ 修复完成！');
    console.log('\n说明:');
    console.log('- 触发器现在同时支持中文("买入"/"卖出")和英文("buy"/"sell")格式');
    console.log('- 新订单将自动同步到 trade_records 表');
