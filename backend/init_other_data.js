...
async function initData() {
  try {
    ...
    // 初始化 trade_records (交易记录) - 根据实际表结构
    const tradeRecords = [
      { 
    ...
    for (const t of transactions) {
      await pool.query(
        `INSERT INTO transactions (id, order_id, type, symbol, name, price, quantity, amount, transaction_time, created_at) 
