...
async function testDeleteFlow() {
  ...
  if (deletedItem) {
    console.log(`  ✓ 找到已删除项: date=${deletedItem.date}, deleted=${deletedItem.deleted}`);
  }

  // 步骤7: 验证删除的数据不在查询结果中
  const stillVisible = data2.data?.find(d => d.id === idToDelete);
  const syncedStillVisible = syncedData.find(d => d.id === idToDelete);
  ...
  const tests = {
    '删除成功': deleteResult.success && deleteResult.data.deleted === true,
    '删除后已删除数据不在查询结果中': !stillVisible,
    '同步API正确过滤已删除数据': !syncedStillVisible,
    '数据仍存在于数据库中': deletedItem && deletedItem.deleted === true
  };
