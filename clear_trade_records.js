/**
 * 清空交易记录的数据库数据
 * 此脚本将删除 trade_records 和 orders 表中的所有数据
 * 
 * 使用方法：node clear_trade_records.js
 */
...
if (require.main === module) {
  ...
  rl.question('⚠️  确认要清空所有交易记录数据吗？此操作不可逆！(yes/no): ', (answer) => {
    rl.close();
    
    if (answer.toLowerCase() === 'yes') {
      clearTradeRecords().catch(console.error);
    } else {
      console.log('❌ 操作已取消。\n');
      process.exit(0);
    }
...
module.exports = { clearTradeRecords };
