      ...
      installTransactionsTrigger: async () => {
        ...
        try {
          ...
          const sqlParts = [
            ...
            "      account_name, account_number, amount, transaction_type, ",
            "      category, description, trade_number, created_at, updated_at",
            "    ) VALUES (",
            "      account_name, account_num, transaction_amount, transaction_type,",
            "      transaction_category, ",
            "      NEW.order_type || ' ' || NEW.stock_code || '(' || NEW.stock_name || ') ' || NEW.quantity || '股',",
            "      NEW.trade_number, NEW.created_at, NEW.updated_at",
            "    );",
            "  END IF;",
            "  ",
            "  -- 更新账户余额（仅处理最新的操作）",
